...
gulp.task('json', function () {
    return gulp.src([appDir + '/json/*.json'])
        .pipe(gulp.dest(destDir+ '/json'))
});
gulp.task('templates', function () {
    return gulp.src([appDir + '/templates/*.*'])
...
gulp.task('build', function (cb) {
    runSequence(['img', 'css', 'json', 'html', 'templates', 'bower_components', 'templateCache', 'build-es6'], 'injects', cb);
});
